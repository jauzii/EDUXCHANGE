...
export default function AdminQuizzesEdit({ course, quiz }) {
    const { data, setData, put, processing, errors } = useForm({
        judul: quiz.judul,
        urutan: quiz.urutan,
        konten: quiz.konten,
        soal: quiz.soal.length > 0 ? quiz.soal : [soalKosong()],
    });

    const submit = (event) => {
        event.preventDefault();
        put(route('admin.courses.quizzes.update', [course.id, quiz.id]));
    };
            ...
            <div className="py-8">
                <div className="mx-auto max-w-3xl space-y-8 px-4 sm:px-6 lg:px-8">
                    <section className="rounded-[2rem] bg-slate-950 p-8 text-white shadow-xl shadow-slate-200">
                        ...
                        <h1 className="mt-2 text-3xl font-extrabold tracking-tight">Edit Modul</h1>
